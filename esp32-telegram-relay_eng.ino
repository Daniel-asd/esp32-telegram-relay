...
const int   relayPin     = 5;                   // GPIO pin for the relay
const char* scheduleFile = "/data.json";        // file in SPIFFS
...
long gmtOffset_sec      = 0;   // offset from UTC in seconds (e.g., +3 hours -> +10800)
int  daylightOffset_sec = 0;   // DST adjustment (always 0 here)
...
String adminChatId       = "YOUR_ID"; // default admin ID
...
void handleNewMessages(int numNewMessages) {
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (!isAllowedUser(fromId)) {
      bot.sendMessage(fromId, "Sorry, access denied.", "");
      continue;
    }
    ...
    if (waitState == WAIT_ON_TIME && fromId == adminChatId) {
      ...
      if (parseTime(text, h, m, s)) {
        ...
        bot.sendMessage(fromId, "On time set: " + formatTime(h, m, s) +
                                "\nNow send off time (HH:MM:SS)", "");
      } else {
        bot.sendMessage(fromId, "Invalid format! Use HH:MM:SS", "");
      }
      continue;
    }
    // Admin sets "off" time
    if (waitState == WAIT_OFF_TIME && fromId == adminChatId) {
      ...
      if (parseTime(text, h, m, s)) {
        ...
        waitState = NONE;
        saveData();
        bot.sendMessage(fromId, "Off time set: " + formatTime(h, m, s) + "\nSchedule saved.", "");
      } else {
        bot.sendMessage(fromId, "Invalid format! Use HH:MM:SS", "");
      }
      continue;
    ...
    if (text.startsWith("/add ") && fromId == adminChatId) {
      ...
      if (addUser(newId)) {
        saveData();
        bot.sendMessage(fromId, "User " + newId + " added.", "");
      } else {
        bot.sendMessage(fromId, "Failed to add user (exists or limit).", "");
      }
      continue;
    }
    ...
    if (text.startsWith("/remove ") && fromId == adminChatId) {
      ...
      if (remId == adminChatId) {
        bot.sendMessage(fromId, "Cannot remove admin.", "");
      } else if (removeUser(remId)) {
        saveData();
        bot.sendMessage(fromId, "User " + remId + " removed.", "");
      } else {
        bot.sendMessage(fromId, "User " + remId + " not found.", "");
      }
      continue;
    }
    // Command handling
    if (text == "/start" || text == "/menu") {
      ...
      bot.sendMessage(fromId, "Relay is ON", "");
    }
    else if (text == "/off") {
      digitalWrite(relayPin, LOW);
      bot.sendMessage(fromId, "Relay is OFF", "");
    }
    else if (text == "/time") {
      bot.sendMessage(fromId, "Current time: " + getCurrentTimeStr(), "");
    }
    else if (text == "/ip") {
      bot.sendMessage(fromId, "Web panel: http://" + WiFi.localIP().toString(), "");
    }
    else if (text == "/admin") {
      if (fromId == adminChatId) {
        sendAdminMenu(fromId);
      } else {
        bot.sendMessage(fromId, "Access denied.", "");
      }
    }
    else if (text == "/list_users" && fromId == adminChatId) {
      String list = "Allowed Users:\n";
      ...
      bot.sendMessage(fromId, "Send on time (HH:MM:SS)", "");
    }
    else {
      bot.sendMessage(fromId, "Unknown command. Use /menu", "");
    }
  }
}
...
void handleRoot() {
  ...
  String html = R"rawliteral(
    ...
    <body>
      <header>
        <h2>ESP32 Admin Panel</h2>
      </header>
      <main>
        <section>
          <div class="section-title">Admin Chat ID</div>
          <form method="POST" action="/setAdmin">
            <label>Current: )rawliteral"
      + adminChatId +
    R"rawliteral(</label>
            <input type="text" name="adminid" placeholder="New Admin Chat ID">
            <button type="submit">Save</button>
          </form>
        </section>
        <section>
          <div class="section-title">Allowed Users</div>
          <ul>
  )rawliteral";
  ...
  html += R"rawliteral(
          ...
          <form method="POST" action="/addUser">
            <label for="userid_add">Add Chat ID:</label>
            <input type="text" id="userid_add" name="userid" placeholder="Chat ID to add">
            <button type="submit">Add</button>
          </form>
          <form method="POST" action="/removeUser">
            <label for="userid_remove">Remove Chat ID:</label>
            <input type="text" id="userid_remove" name="userid" placeholder="Chat ID to remove">
    ...
    R"rawliteral(</p>
          <form method="POST" action="/setSchedule">
            ...
            <input type="text" id="ontime" name="ontime" placeholder="e.g. 07:30:00">
            <label for="offtime">Off time (HH:MM:SS):</label>
            <input type="text" id="offtime" name="offtime" placeholder="e.g. 22:00:00">
            <button type="submit">Save</button>
          </form>
        </section>
        <section>
          <div class="section-title">UTC Offset</div>
          <p>Current: )rawliteral"
      + (gmtOffset_sec >= 0 ? "+" : "-")
      + formatTime(abs(gmtOffset_sec)/3600, (abs(gmtOffset_sec)%3600)/60, 0) +
    R"rawliteral(</p>
          <form method="POST" action="/setTZ">
            <label for="tz">Enter UTC offset (±HH:MM):</label>
            <input type="text" id="tz" name="tz" placeholder="e.g. +03:00">
            <button type="submit">Save</button>
          </form>
        </section>
      </main>
